    ...
    public class BookDAL
    {
        ...
        public void updateBookreviewDAL(string ISBN,int starcount)
            {   
                ...
                MySqlCommand cmd = new MySqlCommand("UPDATE books SET reviewCount = @rc, starCount = @sc WHERE ISBN = @ISBN", conn);
            rc += 1;
            sc += starcount;
                cmd.Parameters.Add(new MySqlParameter("@rc", rc));
                cmd.Parameters.Add(new MySqlParameter("@sc", sc));
                cmd.Parameters.Add(new MySqlParameter("@ISBN", ISBN));
